...
**路径与域**：
- PRD 草稿写入 `.ai/prd/<domain>/[feature].md`；draft user story 写入 `.ai/user-stories/<domain>/[feature].md`
- `<domain>` 只能是 `auth`、`billing`、`core`、`integration`
- `/t-prd` 不写入 `docs/prd/` 和 `docs/user-stories/`；若父目录缺失，仅在目标域已明确时创建对应 `.ai/` 子目录
...
写入 PRD 草稿前必须完成轻量澄清门禁：先研究事实，再对真正阻塞 PRD 的判断点逐一追问，直到足以形成可审阅的产品语义。

提问规则：
- 用户已在命令或当前对话中给出的信息不重复追问；先读取现有文档和代码上下文，不询问可直接查明的事实
...
门禁规则：
- Discover first：先读取现有文档、代码、tech research 和产品 guide；能查明的事实不得问用户。
...
上游输入（可选，如果存在会提升质量；工作流程各步骤按此清单读取，先索引、后明细，`.ai/decision-log` 和 `.ai/decision` 必须最先检查）：
...
按 Input Contract 读取上游输入。根据已发布/候选用户故事、草稿/正式 PRD 和需求语义推断目标域。无法推断时用 `AskUserQuestion` 询问一次。
...
从 `.ai/decision/$ARGUMENTS.md` 提取目标用户、问题陈述、范围方向、已确认产品决策、仍阻塞 PRD 的问题和给 PRD 的 Handoff。若 Verdict 不允许继续进入 PRD，停止并提示回到 `/t-decision`。
...
是否需要后端 API / 前端实现优先从技术研究报告或代码结构推断，不作为默认提问项。
...
基于已收集信息建立临时 `PRD Grill Snapshot`，按"提问与澄清门禁"检查是否满足最低条件。
