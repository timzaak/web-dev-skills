...
## 目标
- 像“做梦后整理记忆”一样，把 PRD、用户故事、设计、任务、Demo 注释、实现事实和项目结构重新收敛成当前可信上下文。
...
`t-dream` 不是单纯的“描述准确性排查”，也不是泛文档润色。它的核心问题是：目标项目当前上下文是否干净、可信、结构合理，并且能否继续支撑后续 AI 编程流程。

PRD 治理模式不是泛重写。它只保留当前产品规则、用户可见契约、范围边界和验收目标，删除或降级已经被实现过程替代的临时方案、迁移过程和重复技术细节。
...
默认聚焦 context health：当前上下文是否准确、收敛、可导航、可追踪、结构上可持续。不要把措辞、格式、风格偏好作为主要问题，除非它们导致业务含义失真、查找成本显著增加或后续 agent 容易误用。
...
- 不保留重复旧文档，除非用户明确要求 archive。未明确允许删除时，旧文档默认归档并标注非权威；用户明确允许删除时，才能删除被合并的旧迭代 PRD。
...
2. **设计合并目标**
   ...
   - 写入前输出合并 / 归档 / 删除映射表；如存在待产品裁决的冲突，不得把冲突平均写入权威 PRD。
...
- 当前目标是“整理和重组当前工程上下文”，不是生成新功能、修复实现代码或美化文档。
...
后端模块检查的输入规则：
- 必须能在 `docs/prd/**/*.md` 中定位与 `[module]` 对应的 PRD；不要假设 PRD 平铺在 `docs/prd/[module].md`。
...
- 不要求每个函数、端点、字段都建立细粒度追踪，重点检查产品能力、模块边界、验收路径和关键业务规则。
