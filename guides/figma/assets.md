...
先提取主 frame，再获取同文件 page 清单和稀疏 metadata。用节点名称、类型、宽高和层级筛选候选；不要对整文件逐节点请求 design context，也不要逐张截图预览。开发者给出的 `--asset-url` 直接加入候选，匹配不唯一时列 metadata 证据询问。

## 合成含文字图片

当图片、装饰和文字共同构成不可拆的营销素材时，按节点层级找到覆盖它们的最小父节点并导出。检查节点信息是否包含按钮、动态价格、本地化文案或其它 live UI；存在这些情况先询问。合成图在 manifest 标记 `flattened: true`，页面不得再重复渲染同一文字。
...
- alpha mask 合并只恢复透明度：半透明像素的 RGB 仍是与画布色 `#1e1e1e` 复合后的值。转换时追加 `image --unbake-color 1e1e1e`（与 `--alpha-mask` 连用或对已带正确 alpha 的烘焙图单独使用），脚本按 `真实RGB=(烘焙RGB-(1-a)*背景)/a` 反解；画布色非默认时以实际颜色为准。

## 图片转换

1. 位图转换直接调用 `${CLAUDE_PLUGIN_ROOT}/scripts/figma-assets.py image` 并使用默认编码参数；预期透明图传入 `--alpha-mask <isolated.png> --expect-alpha`。以脚本结果和透明像素检查判断成功，不逐张目视评估压缩效果。
