...
|---|---|---|---|---|
| `[REQ/US-ID]` | [主文档或分端文档章节] | [operationId / 页面 / 模块] | [测试入口或验收路径] | [主文档 §8 行项] |
...
|---|---|---|---|---|
| [operationId] | [GET/POST] | `/api/...` | [用途] | [frontend/flutter/sdk] |
...
> 各端测试细节在分端文档；本节汇总跨端测试入口和主验收路径，供 `/t-task` 直接消费。按 `${CLAUDE_PLUGIN_ROOT}/protocols/verification-evidence-contract.md` 记录验证责任表，业务行为变更不能只填编译/类型检查；尚未执行的场景标明承接阶段。
