...
A Claude Code plugin for Rust + React projects. It turns `Decision -> Tech Research -> PRD -> Design -> Task -> Development -> Acceptance -> Demo` into a reusable workflow, so teams do not need to repeatedly design prompts, switch context manually, or maintain stage boundaries by hand.
