...
- 人负责表达真实意图、裁决取舍、指出细节偏好，并在 Preview、check 和 accept 中持续校准 AI 产物。
...
但 workflow engine 不等于全自动决策机。T-Tools 把流程、上下文和质量门禁结构化，是为了让人更容易介入关键判断，而不是把人的判断从流程里拿掉。尤其是 PRD、设计和验收阶段，人必须把真实想法、取舍理由和细节偏好说出来，AI 才能针对这些信息修正，而不是按模型自己的默认模式生成一份看起来完整但缺少个性的普通产物。

前端 UX 是其中最需要人强硬介入的部分。AI 可以整理流程、列状态、补边界，但它对"什么是好的体验"没有稳定品味；好的 UX 往往来自产品负责人对用户、场景、节奏、信任感和业务语气的选择。因此调用 `t-design` 后，人需要从用户视角重新走一遍前端交互，并明确告诉 AI 哪些体验判断成立，哪些默认方案不接受。
...
HTML Preview 的设计目的，是把 AI 对需求的理解转换成更容易被人快速浏览、质疑和修正的形态。它不是让人一生成就立刻被 Preview 带着走，而是在心里先有一版自己认可的 PRD 后，用 Preview 更快做对照。人不需要先完整读完 Markdown，就能通过 Preview 检查：
...
`t-prd` 生成 HTML 后，人不应马上打开并顺着生成内容阅读。更有效的顺序是先脱离生成物，口述或过一遍自己认可的 PRD：这个需求真正要解决的痛点是什么，哪个用户路径最关键，哪些状态、权限、异常、文案、数据边界或体验细节不能被忽略，哪些地方听起来太泛、太普通或不符合业务语气。然后再看 Preview，用它检查 AI 是否抓住重点，并要求 AI 把这段反馈逐项对照 PRD、user story 和 Preview，明确哪些需要改、哪些只是待确认假设、哪些会影响后续设计。

这一步是 `t-prd` 的关键人类参与点。没有这次先行口述校准，人很容易被 AI 已经生成的叙事带偏；即使 HTML 降低了阅读成本，真正的重点也不一定会自动浮出来。AI 也会倾向于补全一份结构正确但随机性较强、细节平均化的 PRD；模型越自由，越容易用通用经验填空。人的口述反馈把"我真正想要的东西"从脑子里拉到共享上下文里，让 AI 的下一轮修正有靶子。
...
## t-design 的 UX 校准：从用户视角强行指定体验选择

`t-design` 不只是把 PRD 翻译成技术模块、接口、状态和任务前置条件。只要功能涉及前端界面，它还必须承接一次人类主导的 UX 校准。

原因很直接：AI 擅长把交互流程补完整，但不一定知道什么是好的体验。它可能生成一套"功能都在"的页面，却在入口节奏、状态反馈、默认值、错误提示、信息层级、文案语气和操作安全感上显得普通、随机或模板化。好的 UX 很多时候不是唯一正确答案，而是品味选择；这个选择必须由人明确给出。

调用 `t-design` 后，人应站在用户角度口述一遍：

- 用户从哪里进入这个功能，第一眼应该理解什么。
- 用户每一步为什么愿意继续，下一步如何被提示。
- 哪些信息必须前置，哪些信息应该延后或隐藏。
- 加载、空状态、错误、权限不足、危险操作和成功反馈应该如何表现。
- 默认值、批量操作、撤销、确认、保存和离开页面等细节如何影响信任。
- 哪些交互是你认为"好"的，哪些虽然能用但不符合你的 UX 品味。

然后要求 AI 把这段 UX 选择逐项映射回技术设计：组件结构、状态模型、接口时机、错误处理、前端任务和 Demo 验收都要随之调整。否则设计阶段很容易只得到工程上可实现、但用户体验平庸的方案。
...
- 每个 agent 有明确职责和输出契约。
- 每个阶段都有检查或验收。
- 人在关键节点明确表达判断、反馈和取舍，AI 再基于这些反馈进行对照修正。
- 前端 UX 由人明确选择和校准，不能默认交给 AI 按通用模板生成。
