...
影响规划方向的缺口（任务拆分、交付范围、权限/安全边界、数据模型、兼容性策略、验收标准、测试闭环）先按 Topic 查 Decision Log；仍未解决时用 `AskUserQuestion` 获取答案，回答前不得生成或更新任务。回答后先更新 Decision Log 和设计文档。slot item 数量超过 `${CLAUDE_PLUGIN_ROOT}/protocols/task-phase-execution.md` 的默认上限时，同样必须由用户授权后才能写入。
...
- 固定使用 `phase -> slot -> item` 模型；生成串行执行的 item 文件，而不是把 manifest 当执行输入。
...
|---|---|
| `[feature]` | 功能名（必填） |
| `--phase <backend\|frontend\|extension\|miniapp\|flutter\|web-demo\|extension-demo\|flutter-demo>` | 指定阶段生成；未指定时默认选择第一个 active phase |
...
- 按当前 phase 提取设计文档最小相关上下文：主文档（目标范围、交付端范围、跨端契约、测试汇总、文件影响范围）加当前 phase 对应的分端设计文档（backend phase 读 `backend.md`，frontend/web-demo 读 `frontend.md`，extension/extension-demo 读 `extension.md`，flutter/flutter-demo 读 `flutter.md`，miniapp phase 读主文档小程序相关部分，缺失时读主文档可用部分）；未命中相关章节时记录警告，但不得编造设计事实。
- extension-demo 读取 `extension.md` 的浏览器策略及 `${CLAUDE_PLUGIN_ROOT}/guides/extension/demo-testing.md`；extension 适用却缺少分端设计时停止，不套用 Web UI 方案。
...
- 按当前阶段 slot 串行调度相应 agent（映射见下表），每次调度按 `${CLAUDE_PLUGIN_ROOT}/protocols/subagent-dispatch.md` 通过 `Agent` tool 启动。prompt 保持精简：阶段设计摘要、上游 handoff、验证责任表及 verification-evidence-contract 路径、目标 guide/protocol 路径、责任闭环识别要求、输出字段要求、`needs_user_answer` 规则；不得复制 guide、protocol 或 agent 文档中的长篇规则。
...
| extension-demo | dev | extension-demo-dev |
| extension-demo | accept | extension-demo-accept |
...
- `Work` 写具体动作，不写通用工程原则；`Validation` 写目标项目真实命令、脚本或验收证据，不写抽象测试建议。
...
主流程在每个 slot 返回后执行写入前硬校验（失败则终止该 slot）：

- `slot` 与被调度 agent 匹配；item 必填字段齐备、ID 唯一；manifest 按返回的 items 顺序覆盖全部 items，表格顺序与 `items` 列表一致，且 manifest、item 文件路径和 `.state.json` 计划一致
- 符合 task-phase-execution 的最小 item 结构、拆分阈值、Slot Item Count Limits、测试集中执行和 Backend Test Item Types 规则（backend/test runner 全量命令缺升级原因时拒绝写入）
- `self_check` 存在且未声明未解决 P0/P1；slot agent 已说明 item 的责任闭环
- 存在把技术层、文件类型或实现步骤当成唯一拆分依据、重复验证命令或不可独立验收的过度拆分时拒绝写入
- item 数量超限且无用户授权证据时拒绝写入
- 先写入当前 slot manifest 和 item 文件，再继续调用下游 slot；当前阶段 slot 齐备后再生成 `index.md`；文档写入与 `.state.json` 更新保持同轮完成
...
- 在任务文档中复制 guide/protocol/subagent 的长篇指南，或写与当前交付无关的通用实现建议、培训内容、风格偏好

## 失败处理

- 设计文档不存在：提示先运行 `/t-design [feature]`
- slot agent 返回结构不合格（必填字段缺失、manifest 顺序不一致、`self_check` 缺失、覆盖不完整、类型非法、明显过度拆分或超限无授权）：拒绝写入成功状态，要求重新生成或合并该 slot
- slot agent 生成失败：终止本次任务生成，不写入该 slot 的成功状态，并返回失败 agent 与失败原因
- slot agent 返回 `needs_user_answer`：按决策纪律提问，回答前不写入该 slot；回答后先同步设计/规划依据，再重新生成该 slot

## 示例

```bash
/t-task <feature> --phase backend   # 生成 backend 阶段任务
/t-task <feature>                   # 未指定 phase 时选择第一个 active phase
```
