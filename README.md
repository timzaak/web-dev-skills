...
- Figma 工作流包含四个独立入口，不进入 Decision→Release 主链路：先用 `/t-tools:t-figma-assets <figma-url> <target-file>` 发现、下载并转换零散图片/视频素材，再用 `/t-tools:t-figma-impl <figma-url> <target-file>` 二次重建设计结构并还原整页；需要动效交互时运行 `/t-tools:t-figma-ux <figma-url> <target-file>`，以原型证据和提炼自迪士尼十二原则的动效基准实现界面动效；需要局部精修时运行 `/t-tools:t-figma-fix <figma-node-url> <target-file> <问题描述>`。四个命令按目标文件共享 `.ai/figma/` session，读取并凝练项目 `docs/figma-rules.md`。
