...
| 问题 | 规范 |
| --- | --- |
| 三个命令的产物、关联、manifest、规则记忆和验收门禁 | [Figma 工作流契约](${CLAUDE_PLUGIN_ROOT}/protocols/figma-workflow-contract.md) |
...
标准顺序是 `t-figma-assets -> t-figma-impl -> t-figma-fix（按需）`。三个命令共享由目标文件关联的 session，但职责不互相吞并：assets 不改 UI，impl 负责整页，fix 只精修明确节点。
