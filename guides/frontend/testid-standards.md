...
**适用对象**：frontend-dev、web-demo-dev、前端研发人员。

## 核心原则

- 稳定性优先：避免依赖样式类名、纯文案、DOM 层级。
- 语义化命名：名称应直接表达业务意图。
- 一致性：统一使用 kebab-case。
- 最小必要：仅为有测试价值的元素添加 `data-testid`。
...
- 不添加 `data-testid`，导致测试依赖脆弱选择器。
- 使用无语义命名（如 `btn1`、`button2`）。
- 多元素复用同一 `data-testid`。
- 给装饰元素加 testid，增加维护噪音。
...
- 具体策略见：[Demo 选择器策略主规范](${CLAUDE_PLUGIN_ROOT}/guides/web-demo/selector-strategy.md)
