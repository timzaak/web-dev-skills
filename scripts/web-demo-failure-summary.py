...
def format_summary(failures: list[dict], stats: dict) -> str:
    ...
    for file, tests in sorted(by_file.items()):
        ...
        lines.append(f"  Re-run: `uv run scripts/web-demo-test-runner.py {file}`\n")

    files = sorted(set(failure["file"] for failure in failures))
    lines.append("### Re-run All Failed\n")
    lines.append("```bash")
    for file in files:
        lines.append(f"uv run scripts/web-demo-test-runner.py {file}")
    lines.append("```\n")
    return "\n".join(lines)
...
def main() -> int:
    ...
    if not args:
        print("Usage: python web-demo-failure-summary.py <results.json> [--check-only]")
        return 1
