...
旧版本对每个文件 spawn 一个嵌套 Claude CLI 子进程并串行等待，叠加多轮 diagnose→fix→retest 和长超时，会长时间阻塞发起调用的会话。本 skill 改为**主会话直接驱动循环**：每条命令都是独立、可见的 Bash/Agent 调用，可以随时 Ctrl-C 后用 `/t-tools:t-web-demo-run-all continue` 从断点恢复。

## 使用方式

```bash
/t-tools:t-web-demo-run-all            # 从头运行全部非 live 文件
```

```bash
/t-tools:t-web-demo-run-all continue   # 从最近运行中批次的未完成断点继续
```

```bash
/t-tools:t-web-demo-run-all scan       # 先预扫描聚类，再按唯一根因修复（见"扫描模式"）
```
...
2. **数据隔离**：若还有下一个文件，无论当前文件成功或失败，都先重建 Demo 环境和数据容器：
   ...
   ```
   - 重建失败：执行 `uv run scripts/web-demo-run-all.py block --json <json_report> --error "<简短原因>"` 后中止批次。该命令保留断点且不追加结果。
...
解析最后一行 `Result: {...}`，登记结果（同默认模式 B2 的参数映射）：
...
约束：此阶段禁止 dispatch subagent；文件间数据隔离同默认模式（还有下一个文件就 `demo-stop.py --quiet && demo-start.py`，重建失败时 `block` 后中止）；每文件只输出一行进度。
...
- 扫描模式的预扫描（S2）与聚类（S3）阶段禁止 dispatch subagent；只有 S4 修复才用 `Agent`，且必须先注入角色规范。
- 扫描模式的 `unclusterable` 条目不得静默丢弃，必须逐条人工处理或回退默认模式。
