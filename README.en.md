...
- The standalone Figma workflow has three commands outside the Decision→Release chain: run `/t-tools:t-figma-assets <figma-url> <target-file>` to discover, download, flatten, and convert scattered image/video assets; run `/t-tools:t-figma-impl <figma-url> <target-file>` to reconstruct unreliable flat-canvas structure and implement the complete UI; then use `/t-tools:t-figma-fix <figma-node-url> <target-file> <issue-description>` for scoped visual refinement. The commands share a target-file-associated `.ai/figma/` session and distill validated project rules into `docs/figma-rules.md`.
