...
| 后端按模块 / 场景筛选 | `uv run scripts/backend-test.py -- --module <module> --tests '*<TestClass>'` |
...
后端执行前用 `--module` / `--tests` 对应筛选参数确认选中预期测试；漏选或零用例不算覆盖通过。`mvn verify` 的编译与静态检查、前端 type-check / build 仅是编译或静态检查，不替代测试。

### 环境与结果

- 按实际入口补充必要工具、安装命令和配置：后端 uv / Java / Maven / Docker；前端 Node / npm；Demo 另需 Playwright 浏览器。Vitest 使用 MSW 隔离 API 时无需真实后端。
