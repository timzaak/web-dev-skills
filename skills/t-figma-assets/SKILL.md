...
4. 调用 Figma MCP `download_assets` 导出（`defaultScale` 默认 3，小型非关键图标或装饰 2），临时 URL 立即下载到 session `raw/`，不得直接写入正式代码。视频按 `--video-source` 提供 URL 或项目内文件，缺失则停止。
5. 逐个调用 `${CLAUDE_PLUGIN_ROOT}/scripts/figma-assets.py image|video` 转换并落位；SVG/已有 WebP/GIF 直接复制。同名同 SHA-256 复用，异内容停止。
