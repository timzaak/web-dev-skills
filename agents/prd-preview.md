...
name: prd-preview
description: >
  PRD HTML Preview 生成专家。负责把 Markdown PRD 和用户故事转成同目录、单文件、
...
5. 用 `data-prd-source`、`data-prd-section`、`data-prd-story-id` 标记来源。
...
- 相关用户故事不存在：继续生成，但在 `Assumptions` 中标记缺口。
