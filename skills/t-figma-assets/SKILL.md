...
5. 逐个调用 `${CLAUDE_PLUGIN_ROOT}/scripts/figma-assets.py image|video|svg` 转换并落位，导出倍率和 mask/反烘焙参数按 assets.md 选择；SVG 必须走 `svg` 子命令优化，已有 WebP/GIF 直接复制。正式路径已存在时停止，除非开发者明确要求替换。
