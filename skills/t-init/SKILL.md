...
完整清单见 [references/output-checklist.md](${CLAUDE_PLUGIN_ROOT}/skills/t-init/references/output-checklist.md)（Step 1 创建目录结构和 Step 8 验证时读取），覆盖四组必须产物：`backend/`（workspace 三 crate + 迁移与配置）、`frontend/`（Vite + TanStack 工程）、`scripts/` + `README.md` + `AGENTS.md`、`demo/`（Playwright E2E 与冒烟测试）。
