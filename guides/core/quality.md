...
验收前读取 `${CLAUDE_PLUGIN_ROOT}/protocols/verification-evidence-contract.md`，统一判断业务验证责任、阶段放行、最终交付及证据复用；下文和领域指南中的执行命令按该协议取得或复核证据。
...
- 编译/类型检查失败
- 核心测试失败
- 关键验收流程缺失，或当前负责的必要业务验证失败/未完成（后续阶段承接按验证证据协议）
