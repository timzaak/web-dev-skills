...
- 可用 `uv run scripts/check-test-runner-coverage.py <feature> --layer <backend|frontend|miniapp|demo>` 校验 runner 覆盖关系；backend 做动态命中校验，frontend/miniapp/demo 默认做静态命令覆盖校验。
