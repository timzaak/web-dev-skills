...
- `t-simplify` reviews the current git changes from four parallel read-only angles — reuse, simplification, efficiency, and altitude — then applies the deduplicated fixes directly. It is quality cleanup only and does not hunt for correctness bugs (those belong to `/code-review` and the per-phase accept stages). When the Agent tool is unavailable it falls back to a single-pass review in the main session and states so in the report.
- `t-security-review` performs a focused security review of the pending changes on the current branch: one finder subagent identifies high-confidence vulnerabilities newly introduced by the changes, then parallel filter subagents reject false positives, and only findings with confidence >= 8 reach the report. It reviews only and never fixes code. When the Agent tool is unavailable it falls back to a two-pass review in the main session and states so in the report.
- Before running `t-push`, it is recommended to first run `/code-review --fix`, `/t-tools:t-simplify`, and `/t-tools:t-security-review` in Claude Code, so the code is independently reviewed, simplified, and security-checked before the final commit; their cleanup is independent of `t-push`'s comment cleaning and will not overwrite each other.

### `t-simplify` Origin

`t-simplify` is a replica of the prompt behind Claude Code's built-in `/simplify` command, as extracted from the Claude Code binary by [Piebald-AI/claude-code-system-prompts](https://github.com/Piebald-AI/claude-code-system-prompts) (MIT): the main two-phase flow comes from the v2.1.154 slash-command prompt, and the single-pass fallback from the v2.1.213 inline-mode prompt for contexts where the Agent tool is unavailable. The four review-angle guidance blocks are runtime-injected variables in the original and were not part of the extraction; this plugin reconstructs them from public behavior in [`protocols/simplify-cleanup-contract.md`](protocols/simplify-cleanup-contract.md), and adds a `simplify-reviewer` subagent role spec plus a `.ai/quality/simplify-*.md` report artifact following this plugin's conventions.

### `t-security-review` Origin

`t-security-review` is a replica of the prompt behind Claude Code's built-in `/security-review` command, extracted verbatim from the embedded strings of a local `@anthropic-ai/claude-code@2.1.232` binary (not a secondhand transcript): the two-stage sub-task orchestration (identify, then filter false positives in parallel per finding), the identification rules, the false-positive exclusion lists, and the discard-below-confidence-8 gate are preserved in [`protocols/security-review-contract.md`](protocols/security-review-contract.md) (the original hard-exclusion list contains a numbering slip with two "16." entries; the contract renumbers them into 18 items with unchanged content). Following this plugin's conventions it adds `security-vuln-finder` and `security-fp-filter` subagent role specs plus a `.ai/quality/security-review-*.md` report artifact.
