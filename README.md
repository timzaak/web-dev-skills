...
面向 Java Spring Boot + React 项目的 Claude Code plugin。它把 AI 编程拆成一条可执行、可恢复、可验收的工程工作流：
...
PRD、技术预研和设计阶段需要人的明确校准。不会口播时，直接打开 [莫要偷懒](human/speech-template.md)，按里面的标题念：重点说清楚这件事真正重要在哪里、用户故事不能跑偏的点、产品边界和取舍、用户最需要看懂什么、权限/数据/高风险操作、技术预研必须查清楚什么，以及你最担心的风险和未知。AI 吞吐这段口播后，应先输出重点理解，评估可执行性、可行性和遗漏点，再生成或修正 PRD、技术预研和设计输入。`/t-tools:t-prd` 后，先脱离生成物口述你认可的 PRD，再看 HTML Preview 并让 AI 对照修正。`/t-tools:t-design` 后，从用户视角明确 UX 入口、路径、反馈、默认值和错误状态，再让 AI 修正技术设计。
