...
1. `.ai/figma/<id>/spec.json` — 设计规格快照，唯一视觉真相源。
2. `.ai/figma/<id>/context.md` — 栈声明 + token/动效/组件清单。
3. `.ai/figma/<id>/conflicts.json` — 已确认的 token 冲突；首次可不存在，完成前必须创建。
4. `.ai/figma/<id>/delta-report.json` — 仅回环时存在。
5. `${CLAUDE_PLUGIN_ROOT}/guides/figma-restore/measurement.md` — 理解什么会被测量。
6. 目标项目对应栈的 guide（如 frontend：`${CLAUDE_PLUGIN_ROOT}/guides/frontend/*`）。
...
5. 把 spec 与 project token 的已确认冲突写入 `.ai/figma/<id>/conflicts.json`；无冲突时写 `[]`，已消除的冲突必须删除。
6. 跑该栈的验证（见「完成前验证」）。
