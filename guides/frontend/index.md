...
| 只读验收、API 一致性、Demo-first 测试合规 | [quality.md](${CLAUDE_PLUGIN_ROOT}/guides/frontend/quality.md) |
| `data-testid` 命名与覆盖范围 | [testid-standards.md](${CLAUDE_PLUGIN_ROOT}/guides/frontend/testid-standards.md) |

## 使用规则

- `development.md` 是 frontend 的事实型主规范，其他页面不应重复定义第二套架构真相。
- `patterns.md` 只记录目标项目已采用的高频实现模式，不替代真实代码。
