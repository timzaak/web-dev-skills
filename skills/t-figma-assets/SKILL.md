...
4. 调用 Figma MCP `download_assets` 导出（`defaultScale` 默认 3，小型非关键图标或装饰 2），临时 URL 立即下载到 session `raw/`，不得直接写入正式代码。预期透明的组合节点还要调用 `get_screenshot(contentsOnly: true)`，将隔离截图保存为 alpha mask；视频按 `--video-source` 提供 URL 或项目内文件，缺失则停止。
5. 逐个调用 `${CLAUDE_PLUGIN_ROOT}/scripts/figma-assets.py image|video` 转换并落位；预期透明的图片调用 `image --alpha-mask <isolated.png> --expect-alpha`，避免 Figma 将画布色烘焙为不透明背景。SVG/已有 WebP/GIF 直接复制。正式路径已存在时停止，除非开发者明确要求替换。
6. 汇总脚本 JSON 输出写 `assets-manifest.json`（字段按契约；无素材也写 `[]`），删除 `raw/`，session stage 设为 `assets`。

## 门禁

- 缺 ffmpeg/ffprobe 时停止并给安装提示；失败不写半成功 manifest。
