...
- Preview 为表达内容所做的推断只能是不改变来源语义的可视化假设，并必须显式标注；需要用户裁决的问题不得由 Preview 静默承载。
...
- 注意项区域，使用 `data-doc-section="Attention"` 标记；如没有风险、验证动作或可视化假设，写“无”。
...
- 颜色：只表达状态，不做装饰（Tufte 数据墨水比：最大化承载信息的墨水，最小化装饰墨水）。红色表示阻塞或高风险，黄色表示验证动作，绿色表示已满足，灰色表示背景信息。
